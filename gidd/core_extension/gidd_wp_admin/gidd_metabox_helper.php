...
function ___field( $field, $metabox_id = "" ){
	...
	if ( $field->value == "" ){
		if ( $metabox_id != "" ){
			...
			$field->value = isset( $option[ "$fid" ] ) ? $option[ "$fid" ] : "";
		}
	}
...
function ___metabox( $appname, $title, $callback, $post_type = "", $context = "", $priority = "" ){
	...
	$meta->post_type = ( $post_type == "" ) ? "post" : $post_type;
	$meta->context = ( $context == "" ) ? "advanced" : $context;
	$meta->priority = ( $priority == "" ) ? "default" : $priority;
...
function gidd_create_metabox( $meta ){
	...
	//save action
	add_action( 'save_post', 'gidd_save_post' );	
	return $id;
}
...
function gidd_save_post( $post_id ){
	
	//post id
	$mid = isset( $_POST['metabox_id'] ) ? $_POST['metabox_id'] : "";
	...
	/*** ok, authenticated ***/
	//get metabox id
	$meta = isset ( $_POST[ "$mid" ] ) ? $_POST[ "$mid" ] : "";
