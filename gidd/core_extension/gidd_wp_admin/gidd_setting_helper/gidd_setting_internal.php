...
function gidd_add_settings_section( $section, $fields ){
	...
	if ( is_array( $fields ) ){
		...
		foreach ($fields as $field ){
			...
			$section .= "add_settings_field('$fid', '$label', 'gidd_admin_field', '$page_slug', '$id', array('$callback', '$fid', '$id', '$description', '$class', '$val'));";
		}
	}
...
function gidd_admin_field( $args ){
	...
	$field->value = $args[5]; //set the value from application/template
	
	if ( isset( $option ) && ( $option != "" ) )
		$field->value = isset( $option[$args[1]] ) ? $option[$args[1]] : ""; //get the saved setting from database
...
function gidd_sanitize_option( $input ){
	...
	foreach ( $output as $key => $val ){
		...
		//re-assign the value to the accept array
		$accept["$key"] = $val;
