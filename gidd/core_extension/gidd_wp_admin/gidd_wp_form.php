...
class Gidd_WP_Form extends Gidd_Data{
	...
	function label( $field ){		
		$class = $field->_class;		
		if( $field->callback == "text" ){ $class = ""; }
		$label = '<label class="gidd-label '. $class .'" id="label'. $field->id .'" for="'. $field->name .'">'. $field->label .'</label>';
	...
	function text( $field ){
		...
		$text .= '<input type="text" id="'. esc_attr ( $field->id ) .'" name="'. esc_attr ( $field->name ) .'" value="'. stripslashes( esc_html ( $field->value ) ) .'" class="'. esc_attr( "regular-text " . $field->_class ) .'" '. $field->attribute .' />';
	...
	function password( $field ){
		...
		$password .= '<input type="password" id="'. esc_attr ( $field->id ) .'" name="'. esc_attr ( $field->name ) .'" value="'. stripslashes( esc_html ( $field->value ) ) .'" class="'. esc_attr( $field->_class ) .'" autocomplete="'. esc_attr( "off" ) .'" '. $field->attribute .' />';
	...
	function textarea( $field ){
		...
		$textarea .= '<textarea class="'. esc_attr( $field->_class ) .'" id="'. esc_attr ( $field->id ) .'" name="'. esc_attr ( $field->name ) .'" rows="'. esc_attr( '5' ) .'" cols="'. esc_attr( '90' ) .'" '. $field->attribute .'>' . stripslashes( esc_textarea ( $field->value ) ) . '</textarea>';
	...
	function checkbox( $field ){
		...
		$chk .= '<input class="'. esc_attr( $field->_class ) .'" type="checkbox" id="'. esc_attr ( $field->id ) .'" name="'. esc_attr ( $field->name ) .'" value="'. esc_attr( '1' ) .'" '. $checked .' '. $field->attribute .' /> ';
	...
	function radio( $field ){
		...
		foreach ($items as $key => $val){
			...
			$radio .= '<input '. $checked .' type="radio" class="'. esc_attr ( $field->id . " " . $field->_class ) .'" name="'. esc_attr ( $field->name ) .'" value="'. esc_html ( $key ) .'" '. $field->attribute .' /> ';
	...
	function select( $field ){
		...
		if ( $field->multiple == "multiple" ){			
			...
			$select .= '<select '. esc_attr ( $field->multiple ) .' id="'. esc_attr ( $field->id ) .'" name="'. esc_attr ( $field->name ) .'[]" class="multiselect '. esc_attr( $field->_class ) .'" '. $field->attribute .'>';
			...
			$select .= '<select id="'. esc_attr ( $field->id ) .'" name="'. esc_attr ( $field->name ) .'" class="'. esc_attr( $field->_class ) .'" '. $field->attribute .'>';
