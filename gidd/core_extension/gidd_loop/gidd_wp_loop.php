...
abstract class Gidd_WP_Loop extends Gidd_Data{
	...
	protected function loop_method( $method ) {
		...
		switch ( $method ) {
			...
			case "thumb_only"	:	$this->_show_thumb( $this->thumbnail['link'] ); break;
			case "none"			:	break;
			
			default				:	$wl =& $this->content;
	...
	protected function _get_thumbnail_with_size( $link ){
		if ( $link == "show-link" ) {
			...
			the_post_thumbnail( array( $this->thumbnail['dimension']['width'], $this->thumbnail['dimension']['height'] ), array( 'title'=>'thumbnail','alt'=>'thumbnail', 'class' => 'my_thumb' ) );
			echo ( '</a>' );
		} else {
			the_post_thumbnail( array( $this->thumbnail['dimension']['width'], $this->thumbnail['dimension']['height'] ), array( 'title'=>'thumbnail','alt'=>'thumbnail', 'class' => 'my_thumb' ) );
		}
	...
	protected function _show_thumb( $link = "" ) {
		...
		if ( count( $this->thumbnail['dimension'] ) > 0 )
	...
	protected function show_thumbnail( $position = "before-title" ) {
		...
		if ( is_array( $this->thumbnail ) ) {
			if( $this->thumbnail['position'] == $position ) {
				$this->_show_thumb( $this->thumbnail['link'] );				
			}
		}
	...
	protected function show_read_more( $position = "" ) {
		
		if ( isset( $this->readmore['position'] ) ):
			if ( $this->readmore['position'] == $position ){
				echo ( '<p class="readmore"><a href="' ); the_permalink(); 		
				echo ( '" title="continue reading this post" class="'.  $this->readmore['class'] .'">'. '<span>' .  $this->readmore['text'] ."</span></a></p>" );
			}
		endif;
		
	}
	
	protected function show_avatar(){
	
		if ( intval( $this->avatar ) > 0 ){
			echo '<div class="avatar">';
			echo get_avatar( get_the_author_meta('ID'), $this->avatar );
			echo '</div>';
		}
